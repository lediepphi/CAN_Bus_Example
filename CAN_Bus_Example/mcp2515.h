...
#ifndef MCP_USED_CLOCK
#define MCP_USED_CLOCK MCP_8MHZ
#endif
...
class MCP2515
{
  ...
  private:
    ...
    static const uint32_t SPI_CLOCK = 8000000; // 10MHz
  ...
  public:
    ...
    ERROR setBitrate(const CAN_SPEED canSpeed, const MCP_CLOCK canClock);
    ...
    ERROR sendMessage(const TXBn txbn, const struct Can_MsgBufferType *frame);
    ERROR sendMessage(const struct Can_MsgBufferType *frame);
    ERROR readMessage(const RXBn rxbn, struct Can_MsgBufferType *frame);
    ERROR readMessage(struct Can_MsgBufferType *frame);
