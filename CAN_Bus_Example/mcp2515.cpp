...
{
  return setBitrate(canSpeed, MCP_USED_CLOCK);
}

MCP2515::ERROR MCP2515::setBitrate(const CAN_SPEED canSpeed, MCP_CLOCK canClock)
{
...
MCP2515::ERROR MCP2515::sendMessage(const TXBn txbn, const struct Can_MsgBufferType *frame)
{
...
MCP2515::ERROR MCP2515::sendMessage(const struct Can_MsgBufferType *frame)
{
  if (frame->dlc > CAN_MAX_DLC) {
    return ERROR_FAILTX;
  }
...
MCP2515::ERROR MCP2515::readMessage(const RXBn rxbn, struct Can_MsgBufferType *frame)
{
...
MCP2515::ERROR MCP2515::readMessage(struct Can_MsgBufferType *frame)
{
  ERROR rc;
